...
from .forms import ConsultaForm, AvisoAusenciaForm
...
class AvisoListView(AdminRequiredMixin, ListView):
    model = AvisoAusencia
    template_name = 'consultas/listar_avisos.html'
    context_object_name = 'avisos'
    ordering = ['-data_inicio']


class AvisoCreateView(AdminRequiredMixin, CreateView):
    model = AvisoAusencia
    form_class = AvisoAusenciaForm
    template_name = 'consultas/form_aviso.html'
    success_url = reverse_lazy('listar_avisos')

    def form_valid(self, form):
        messages.success(self.request, 'Aviso de ausência criado com sucesso!')
        return super().form_valid(form)


class AvisoUpdateView(AdminRequiredMixin, UpdateView):
    model = AvisoAusencia
    form_class = AvisoAusenciaForm
    template_name = 'consultas/form_aviso.html'
    success_url = reverse_lazy('listar_avisos')
    ...
    def form_valid(self, form):
        messages.success(self.request, 'Aviso de ausência atualizado com sucesso!')
        return super().form_valid(form)


class AvisoDeleteView(AdminRequiredMixin, DeleteView):
    model = AvisoAusencia
    template_name_= 'consultas/confirmar_exclusao.html'
    success_url = reverse_lazy('listar_avisos')

    def delete(self, request, *args, **kwargs):
        messages.success(request, 'Aviso de ausência excluído com sucesso!')
        return super().delete(request, *args, **kwargs)
